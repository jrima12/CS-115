'''
CS 115, Inheritance Activity - SOLUTION - BASIC VERSION
'''
...
class Employee(object):
    ...
    def __str__(self):
        '''This is implemented using Python's string formatting operator.'''
        return 'Employee: %s %s\n  Title: %s\n  Hours per week: %d\n' \
...
class Manager(Employee):
    ...
    def __init__(self, first_name, last_name, title, hours_per_week, \
        ...
        try:
            self.bonus_percent = float(bonus_percent)   # convert the input string 
        except:
            print("There was an error, so no bonus.")
            self.bonus_percent = 0.0
        if self.bonus_percent < 0:
            print("Negative bonuses not allowed, so no bonus.")
            self.bonus_percent = 0.0
    ...
    def get_total_compensation(self):
        return (1 + self.bonus_percent / 100) * Employee.get_total_compensation(self)
        # Alternatively:
        # return (1 + self.bonus_percent / 100) * super().get_total_compensation()
